...
} // Hard coded for the glowing star 
    ...
    Entity **collidableEntities, int collisionCheckCount)
{
    ...
    if (mEntityType == PLAYER || mEntityType == NPC) // Changed to fix the bug of the ball not moving
    {
    ...
    if (mTextureType == ATLAS && GetLength(mMovement) != 0 && mEntityType == NPC) 
        animate(deltaTime);
}
